...
class InteractiveManager:
    ...
    def run_s2m(self):
        ...
        inputs["height"] = h
        inputs["width"] = w
        img, _ =  self.transforms.get_transform(self.image).apply_image(self.image)
        inputs["image"] = torch.as_tensor(np.ascontiguousarray(img.transpose(2, 0, 1)))

        fg_scrbs = self.transforms.get_transform(self.p_scrb).apply_segmentation(self.p_scrb)
        bg_scrbs = self.transforms.get_transform(self.n_scrb).apply_segmentation(self.n_scrb)
...
parser.add_argument('--model', default="../interactive_output_multiscale/model_final.pth")
...
model.eval()
cfg.MODEL.WEIGHTS = "interactive_output_multiscale/model_final.pth"
...
manager = InteractiveManager(model, image, mask)
