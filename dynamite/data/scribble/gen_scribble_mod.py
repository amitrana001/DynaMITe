...
def get_background_scribble(img, gt, use_robot = True):

    img = img >= 0
    gt = gt > 128

    bg = (img & ~gt).astype(np.uint8)*255
    
    if use_robot:
        # The robot is similar to the DAVIS official one
        bg_scr = robot.interact(bg).astype(np.uint8)
        bg_scr = cv2.dilate(bg_scr, np.ones((3,3)))
        return bg_scr
    else:
        # Opening operator to remove noises
        opening_size = np.random.randint(5, 20)
        bg = cv2.morphologyEx(bg, cv2.MORPH_OPEN, disk_kernel(opening_size))
        # fn = cv2.morphologyEx(fn, cv2.MORPH_OPEN, disk_kernel(opening_size))
        ...
        # Sometimes we just draw scribbles referring only to the GT but not the given mask
        if np.random.rand() < 0.3 or (scribbles[0].sum() == 0):
            for i, m in enumerate([gt.astype(np.uint8), (~gt).astype(np.uint8)]):
                if m.sum() < 100:
                    continue
                this_scribble = get_curve_scribble(m, max_srb=5, sort=False)
        ...
        return scribbles[0].astype(np.uint8)
...
if __name__ == '__main__':
    ...
    bg_scribble = get_background_scribble(img, gt, False)

    cv2.imwrite('bg_scribble_mivos3.png', bg_scribble*255)
