...
__all__ = ["COCOEvaluationDatasetMapper"]
...
class COCOEvaluationDatasetMapper:
    """
        ...
        if "annotations" in dataset_dict:
            ...
            if hasattr(instances, 'gt_masks'):
                ...
                for gt_mask in gt_masks:
                    ...
                    fg = get_scribble_eval(np.asarray(gt_mask).astype(np.uint8)*255, bg = False)
                ...
                dataset_dict["bg_scrbs"] = get_scribble_eval(np.asarray(all_masks).astype(np.uint8)*255, bg = True)
                # dataset_dict["bg_scrbs"] = torch.zeros_like(all_masks).unsqueeze(0)
