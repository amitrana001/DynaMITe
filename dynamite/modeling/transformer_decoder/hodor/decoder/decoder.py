...
class Decoder(nn.Module):
    ...
    def forward(self, queries: Tensor, fmaps: Dict[int, Tensor], query_embed: Tensor,
        ...
        query_embed = repeat(query_embed, "C -> B I T C", B=bs, I=num_inst, T=t)

        with torch.no_grad():
            pos_encodings = self.pos_encoding_gen(fmap)

        for layer in self.attn_layers:
            fmap = layer(queries=queries,fmap=fmap, pos_fmap=pos_encodings,
                         pos_queries=query_embed, query_mask=query_mask)
