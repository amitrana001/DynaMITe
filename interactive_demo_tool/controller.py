...
class InteractiveController:
    ...
    def set_image(self, image):
        ...
        self.fg_scrbs = [np.zeros(image.shape[:2], dtype = np.uint8)]
        self.bg_scrbs = [np.zeros(image.shape[:2], dtype = np.uint8)]
    ...
    def add_click(self, x, y, bg_click =False, inst_num=1):
        ...
        if bg_click:
            ...
            # if len(self.bg_scrbs)==2:
            #     self.bg_scrbs.append(None)
            #     # print(np.stack(self.bg_scrbs[:-1],0).shape)
            #     self.bg_scrbs[-1] = np.max(np.stack(self.bg_scrbs[:-1],0),0)
            # elif len(self.bg_scrbs)>2:
            #     self.bg_scrbs[-1] = np.max(np.stack(self.bg_scrbs[:-1],0),0)
        else:
            if inst_num > len(self.fg_scrbs):
                self.fg_scrbs.append(np.zeros(self.image.shape[:2], dtype = np.uint8))
            cv2.circle(self.fg_scrbs[inst_num-1], (x, y), radius=5, color=(1), thickness=-1)
        
        if len(self.bg_scrbs)==0:
            self.bg_scrbs.append(np.zeros(self.image.shape[:2], dtype = np.uint8))
        self.p_scrbs = np.stack(self.fg_scrbs,axis=0)
        for x in self.bg_scrbs:
            print("x:",x.shape)
        ...
        # fg_scrbs = self.transforms.get_transform(self.p_scrbs[::-1]).apply_segmentation(self.p_scrbs[::-1])
        # bg_scrbs = self.transforms.get_transform(self.n_scrbs[::-1]).apply_segmentation(self.n_scrbs[::-1])
        # fg_scrbs = torch.from_numpy(fg_scrbs).unsqueeze(0).float()
        # bg_scrbs = torch.from_numpy(bg_scrbs).unsqueeze(0).float()
        self._inputs["fg_scrbs"] = fg_scrbs
        self._inputs["bg_scrbs"] = bg_scrbs
        self._inputs["scrbs_count"] =  self._inputs["fg_scrbs"].shape[0] + self._inputs["bg_scrbs"].shape[0]
        ...
        # self.pred = self.predictor(inputs)
        # out = self.pred[1]['pred_masks']
        # # mask_pred_results = outputs["pred_masks"]
        # # upsample masks
        # out = Fn.interpolate(
        #     out,
        #     size=(self.pred[2].tensor.shape[-2], self.pred[2].tensor.shape[-1]),
        #     mode="bilinear",
        #     align_corners=False,
        # )
        # print(out.shape)
        # out = out[0]
        # print(self.pred['instances'].pred_masks.shape)
        # out = out[0] - torch.max(out[1:],dim=0).values
        # out = (out > 0).float()
        # self._result_masks = out.to(dtype= torch.uint8).unsqueeze(0)
        self._result_masks = self.pred['instances'].pred_masks[:self._inputs["fg_scrbs"].shape[0]]
    ...
    def reset_clicks(self):
        ...
        self.p_scrbs = None
        self.n_scrbs = None
        self.fg_scrbs = [np.zeros(self.image.shape[:2], dtype = np.uint8)]
        self.bg_scrbs = []
    ...
    # def get_visualization(self, alpha_blend, click_radius, reset_clicks=False):
    #     # print(self.image)
    ...
    def get_visualization(self, alpha_blend=0.3, click_radius=3, reset_clicks=False,show_only_masks=False):
        ...
        if self.p_scrbs is not None:
            for i, scrb in enumerate(self.p_scrbs):
                # color = [np.random.randint(0, 255), np.random.randint(0, 1), np.random.randint(0, 255)]
                # color = np.array(color_map[total_colors-5*i-4]*255, dtype=np.uint8)
...
def get_color_from_map(index):
    ...
    return color_c
    # def get_visualization(self, alpha_blend=0.3, click_radius=3, reset_clicks=False):
