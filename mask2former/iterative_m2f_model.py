...
class IterativeMask2Former(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.use_multiclass_softmax = use_multiclass_softmax
    ...
    def from_config(cls, cfg):
        ...
        if cfg.ITERATIVE.TRAIN.USE_MULTICLASS_SOFTMAX:
            from mask2former.modeling.final_criterion_multi_class import SetFinalCriterion
        ...
        return {
            ...
            "use_multiclass_softmax": cfg.ITERATIVE.TRAIN.USE_MULTICLASS_SOFTMAX,
            # inference
    ...
    def forward(self, batched_inputs, images=None, scribbles=None, num_instances=None,
        ...
        if (images is None) or (scribbles is None) or (num_instances is None):
            images, scribbles, num_instances = preprocess_batch_data(batched_inputs, self.device,
        ...
        if self.training:
            ...
            outputs = self.sem_seg_head(batched_inputs, gt_instances, images, features, num_instances,
            ...
            gt_instances=None
            outputs, mask_features, transformer_encoder_features, multi_scale_features = self.sem_seg_head(batched_inputs, gt_instances, images, features, num_instances,
                                        mask=None, scribbles=scribbles, mask_features=mask_features,
                                        transformer_encoder_features=transformer_encoder_features, 
                                        multi_scale_features=multi_scale_features,
                                        prev_mask_logits=prev_mask_logits)
            processed_results = self.process_results(batched_inputs, images, outputs, num_instances)
            if self.iterative_evaluation:
                return processed_results, outputs, images, scribbles, num_instances, features, mask_features, transformer_encoder_features, multi_scale_features
            else:
    ...
    def interactive_instance_inference(self, mask_cls, mask_pred, num_instances):
        ...
        if self.use_multiclass_softmax:
            mask_pred= mask_pred.softmax(0)
            result.pred_masks = (mask_pred[:num_instances] > 0.5).float()
        else:
            mask_pred = mask_pred[:num_instances]
            result.pred_masks = (mask_pred > 0).float()
        # result.pred_boxes = Boxes(torch.zeros(result.pred_masks.size(0), 4))
        # result.pred_masks = (mask_pred > 0).float()
