...
class SpatioTemporalM2FTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.use_rev_cross_attn:
            ...
            N_steps = hidden_dim // 2
            self.pe_mask_features = PositionEmbeddingSine3D(N_steps, normalize=True)
        ...
        if self.training:
            if self.accumulate_loss:
                ...
                for i in range(num_iters):
                    prev_output = self.iterative_batch_forward(x, mask_features, scribbles, t=i+1, prev_mask_logits=prev_mask_logits)
    ...
    def iterative_batch_forward(self, x, mask_features, scribbles,
        ...
        if self.use_rev_cross_attn:
            ...
            if self.use_deform_conv:
                mask_features = einops.rearrange(mask_features, "B T C H W -> (B T) C H W")
                mask_features = mask_features + self.self_attn_conv_substitute(mask_features.contiguous())
                mask_features = einops.rearrange(mask_features, "(B T) C H W -> B T C H W", T=T)

            with torch.no_grad():
                pos_encodings = self.pe_mask_features(mask_features)
                pos_encodings = einops.rearrange(pos_encodings,"B T C H W -> (T H W) B C")

            B, T, C, H, W = mask_features.shape
            mask_features = einops.rearrange(mask_features,"B T C H W -> (T H W) B C")
            ...
            mask_features = einops.rearrange(mask_features,"(T H W) B C -> B T C H W", H=H, W=W, B=B, T=T).contiguous()
            outputs_class, outputs_mask, _ = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels])
