...
class SpatioTemporalV1M2FTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.use_deform_conv = use_cnn_block
        self.use_rev_attn_mask = use_rev_attn_mask
        if self.use_deform_conv:
            self.self_attn_conv_substitute = nn.Sequential(
                DeformConvBlock(hidden_dim, 1),
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["use_rev_attn_mask"] = cfg.REVERSE_CROSS_ATTN.USE_ATTN_MASK
        ret["use_cnn_block"] = cfg.ITERATIVE.TRAIN.USE_CNN_BLOCK
    ...
    def forward(self, data, targets, images, num_instances, x, mask_features, mask = None,
                 scribbles=None, prev_mask_logits=None):
        ...
        if self.training:
            if self.accumulate_loss:
                ...
                for i in range(num_iters):
                    prev_output = self.iterative_batch_forward(x, mask_features, scribbles, t=i+1, prev_mask_logits=None)
                    # prev_mask_logits = prev_output['pred_masks']
                    mask_features = prev_output['mask_features']
                    ...
                    if self.use_point_clicks:
                        scribbles = get_new_points(data, processed_results, scribbles,
            ...
            ts = num_iters+1 if num_iters>0 else 1
            # if num_iters == 0:
            #     ts = 1
            # else:
            #     ts = num_iters
            outputs = self.iterative_batch_forward(x, mask_features, scribbles, t=ts, prev_mask_logits=prev_mask_logits)
        else:
            outputs = self.iterative_batch_forward(x, mask_features, scribbles, t=1,prev_mask_logits=prev_mask_logits)
        return outputs

    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, t = 1,rev_attn_mask = False):
        ...
        if self.concat_coord_mask_features:
            ...
            mask_embed = einops.rearrange(mask_embed, "B (Q T) C -> B Q T C",T=t)
            # print(t, mask_embed.shape)
            outputs_mask = torch.einsum("bqtc,bchw->bqthw", mask_embed, mask_features)
        ...
        if rev_attn_mask:
            attn_mask = (attn_mask.sigmoid().flatten(3).unsqueeze(1).repeat(1, self.num_heads, 1, 1,1).flatten(0, 1).flatten(1,2) < 0.5).bool()
            attn_mask = attn_mask.permute(0,2,1)
        else:
            # If a BoolTensor is provided, positions with ``True`` are not allowed to attend while ``False`` values will be unchanged.
            attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).unsqueeze(2).repeat(1, self.num_heads, t, 1, 1).flatten(0, 1).flatten(1,2) < 0.5).bool()
        attn_mask = attn_mask.detach()

        return outputs_class, outputs_mask, attn_mask

    def iterative_batch_forward(self, x, mask_features, scribbles,
                                t = 1, prev_mask_logits=None):
        ...
        for i in range(self.num_feature_levels):
            # print(x[i].shape)
            y = einops.repeat(x[i],"B C H W -> B T C H W", T=t)
            size_list.append(y.shape[-2:])
            pos.append(self.pe_layer(y.view(bs, t, -1, size_list[-1][0], size_list[-1][1]), None).flatten(3))
            src.append(self.input_proj[i](y).flatten(3) + self.level_embed.weight[i][None, :, None])

            # print(pos[-1].shape)
            # print(src[-1].shape)
            # NTxCxHW => NxTxCxHW => (TxHW)xNxC
            _,_, c, hw = src[-1].shape
            pos[-1] = pos[-1].view(bs, t, c, hw).permute(1, 3, 0, 2).flatten(0, 1)
            src[-1] = src[-1].view(bs, t, c, hw).permute(1, 3, 0, 2).flatten(0, 1)

        # mask_features = einops.repeat(mask_features, "B C H W -> B T C H W",T=t)
        ...
        if self.random_bg_queries:
            ...
            # print(f"max_scrbs_per_batch:{max_scrbs_batch}")
        ...
        output = repeat(output, "Q N C -> (Q T) N C", T=t)
        # query positional embedding QxNxC
        query_embed = repeat(self.query_embed, "C -> (Q T) N C", N=bs, Q=num_scrbs,T=t)
        # print(output.shape)
        if self.use_pos_coords and (not self.random_bg_queries):
        ...
        outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0],t=t)
        # print(outputs_mask.shape)
        outputs_mask = torch.max(outputs_mask,dim=2).values
        # print(outputs_mask.shape)
        ...
        for i in range(self.num_layers):
            ...
            outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels],t=t)
            outputs_mask = torch.max(outputs_mask,dim=2).values
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
        ...
        if self.use_rev_cross_attn:
            ...
            if self.use_rev_attn_mask:
                _,_, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=(H,W),t=t, rev_attn_mask=True)
                # attn_mask = attn_mask.permute(0,2,1)
                attn_mask[torch.where(attn_mask.sum(-1) == attn_mask.shape[-1])] = False
                # print(t,attn_mask.shape)
                # print("here")
            else:
                attn_mask = None

            if self.use_deform_conv:
                mask_features = mask_features + self.self_attn_conv_substitute(mask_features.contiguous())
            ...
            outputs_class, outputs_mask, _ = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels],t=t)
            # prev_mask_spatio_temp = outputs_mask
            outputs_mask = torch.max(outputs_mask,dim=2).values
            # print(outputs_mask.shape)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
