...
def get_avg_noc(
    ...
    # print(os.getcwd())
    save_results_path = os.path.join("./all_data/evaluations/", cfg.DATASETS.TEST[0], "swin_tiny_scribbles_v2/")
    ...
    stats_file = os.path.join(save_summary_path, f"{model_name}_{max_interactions}_max_click_scrbs_th_final_updated_summary_stats.pickle")
...
def generate_point_all_fp(all_fp, not_clicked_map, radius=5, num_points=1,device='cpu'):
    ...
    all_fp = np.logical_and(not_clicked_map, all_fp)
    try:
        region_scribble = get_curve_scribble(all_fp.astype(np.uint8), max_srb=2)
        # region_scribble = robot.interact(all_fp.astype(np.uint8)).astype(np.uint8)
    except ValueError:
        region_scribble = np.zeros_like(all_fp)
    not_clicked_map[region_scribble>0.5] = False
    return torch.from_numpy(region_scribble).to(device, dtype = torch.uint8), not_clicked_map

from mask2former.data.scribble.gen_scribble import get_curve_scribble
def get_click_on_fp(pred_mask, gt_mask, not_clicked_map, radius=5, device='cpu'):
    ...
    fn_mask = np.logical_and(not_clicked_map, fn_mask)

    try :
        region_scribble = get_curve_scribble(fn_mask.astype(np.uint8),max_srb=2)
        # region_scribble = robot.interact(fn_mask.astype(np.uint8)).astype(np.uint8)
    except ValueError:
        region_scribble = np.zeros_like(fn_mask)
    not_clicked_map[region_scribble>0.5] = False
