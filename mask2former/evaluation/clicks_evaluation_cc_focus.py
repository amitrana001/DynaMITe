...
from mask2former.evaluation.zoomIn import zoomIn
import numpy as np
...
def get_avg_noc(
    ...
    # print(os.getcwd())
    save_results_path = os.path.join("./all_data/evaluations/", cfg.DATASETS.TEST[0], "rev_attn_single_stuff_v06_zoomv01/")
    ...
    with ExitStack() as stack:
        ...
        zoom = zoomIn(cfg, expand_ratio=1.4)
        for idx, inputs in enumerate(data_loader):
            ...
            processed_results, outputs, images, scribbles, num_insts, features, mask_features, transformer_encoder_features, multi_scale_features = model(inputs)
            ...
            while (num_interactions<max_interactions):
                ...
                if use_zoomin:
                    crop_scribbles, crop_mask_features, crop_multi_scale_features, box, full_box = zoom.apply_zoom(
                        inputs, outputs, images, scribbles, mask_features, multi_scale_features
                    )
                    (processed_results_box, outputs_box, images_box, scribbles_box, num_insts,
                    features_box, mask_features_box, transformer_encoder_features_box,
                    multi_scale_features_box)= model(inputs, images, crop_scribbles, num_insts,
                                                    features, crop_mask_features, transformer_encoder_features,
                                                    crop_multi_scale_features)

                    # box_mask = outputs_box['pred_masks'][:,0,:]>0
                    print("box:",box)
                    print(full_box)
                    rmin,rmax,cmin,cmax  = full_box
                    # print(full_box)
                    box_mask = processed_results_box[0]['instances'].pred_masks.to('cpu',dtype=torch.uint8)
                    # print(mask.shape)
                    # mask = mask.to(dtype=torch.uint8).squeeze(0)
                    # mask = np.asarray(mask.to('cpu'),dtype=np.uint8)*255
                    # print(box_mask.shape)
                    box_height = rmax - rmin
                    box_width = cmax - cmin
                    box_mask = torchvision.transforms.Resize(size = (box_height,box_width))(box_mask.unsqueeze(0))
                    box_mask = box_mask.squeeze(0)
                    # print(box_mask.shape)
                    # print(pred_masks.shape)
                    box_mask = torch.logical_and(box_mask, pred_masks[:,rmin:rmax,cmin:cmax]).to(dtype=torch.uint8)
                    pred_masks[:,rmin:rmax,cmin:cmax] = box_mask[0]
                    # import matplotlib.pyplot as plt
                    # # plt.imsave("./output/mask_patch.jpg",mask*255, cmap= 'gray')
                    # os.makedirs('output', exist_ok=True)
                    # # cv2.imwrite('output/%s' % path.basename(args.mask), mask)
                    # cv2.imwrite('output/'+str(inputs[0]['image_id'])+".jpg", mask)
                    # pred_masks = processed_results[0]['instances'].pred_masks.to('cpu',dtype=torch.uint8)
                    # pred_masks = torchvision.transforms.Resize(size = (h_t,w_t))(pred_masks)
                    # for i in range(pred_masks.shape[0]):
                    #     pred_masks[i] = torch.logical_or(pred_masks[i], inputs[0]['fg_scrbs'][i]).to(dtype=torch.uint8)
                    ...
                    ious = compute_iou(gt_masks,pred_masks,ious,iou_threshold)
                    # break
                    save_image(inputs, pred_masks,save_results_path, ious[0], num_interactions, alpha_blend=0.3, box= full_box)
                # break
                ...
                if use_prev_logits:
                    ...
                    images = None
                    processed_results, outputs, images,  scribbles, num_insts, features, mask_features, transformer_encoder_features, multi_scale_features= model(inputs, images, scribbles, num_insts,
                                                                        features, mask_features, transformer_encoder_features,
...
def save_image(inputs, pred_masks, dir_path, iou, num_iter, alpha_blend=0.3,box = None):
    ...
    if box is not None:
        cv2.rectangle(img_write, (box[0], box[2]), (box[1], box[3]), (255,0,0), 2)
