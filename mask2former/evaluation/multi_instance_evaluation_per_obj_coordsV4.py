...
def evaluate(
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            current_ious = np.asarray(ious)
            prev_ious = np.zeros(len(ious))

            current_dious = np.asarray(current_ious)
            prev_dious = np.zeros(len(ious))
            dious_threshold = 0.01
            while (num_interactions<max_iters_for_image and point_sampled):
                ...
                if not point_sampled:
                    ...
                    for i in indexes:
                        if ious[i]<iou_threshold and total_dious[i] >= dious_threshold:
                            (scrbs, is_fg, obj_index, not_clicked_map, coords,
                            fg_click_map, bg_click_map) = get_next_click(pred_masks[i], gt_masks[i], semantic_map, not_clicked_map, fg_click_map,
                                                                        bg_click_map, orig_device, radius, sampling_strategy, padding=True,
                                                                        )
                ...
                if point_sampled:
                    ...
                    prev_dious = current_ious-prev_ious
                    prev_ious, current_ious = current_ious, np.asarray(ious)
                    current_dious = current_ious - prev_ious
