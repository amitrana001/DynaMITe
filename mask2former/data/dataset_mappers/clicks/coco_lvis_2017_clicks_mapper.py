...
from mask2former.data.points.annotation_generator import generate_point_to_blob_masks_eval, generate_point_to_blob_masks

__all__ = ["COCOLVIS2017ClicksDatasetMapper"]
...
class COCOLVIS2017ClicksDatasetMapper:
    """
    ...
    def __call__(self, dataset_dict):
        ...
        if "annotations" in dataset_dict:
            ...
            if hasattr(instances, 'gt_masks'):
                ...
                fg_scrbs, bg_scrbs = generate_point_to_blob_masks(new_gt_masks, all_masks=all_masks)
                ...
                # fg_scrbs, bg_scrbs = generate_point_to_blob_masks(gt_masks, all_masks=all_masks)
                ...
                if self.random_bg_queries:
                    pick = np.random.rand()
                    if pick < 0.80:
                        combined_bg_points = torch.max(bg_scrbs,dim=0).values.to(dtype=torch.uint8)
                        bg_scrbs = torch.cat((bg_scrbs,combined_bg_points.unsqueeze(0)))
