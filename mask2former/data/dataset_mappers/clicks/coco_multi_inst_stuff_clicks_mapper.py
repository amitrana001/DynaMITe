...
__all__ = ["COCOMultiInstStuffClicksDatasetMapper"]

# This is specifically designed for the COCO dataset.
class COCOMultiInstStuffClicksDatasetMapper:
    """
    ...
    def __call__(self, dataset_dict):
        ...
        for segment_info in segments_info:
            ...
            if (not segment_info["iscrowd"]) and (segment_info["area"] > 1000.0):
                if not np.all(~(pan_seg_gt == segment_info["id"])):
                    classes.append(class_id)
                    masks.append(pan_seg_gt == segment_info["id"])
                    isThings.append(segment_info['isthing'])
        isThings = np.asarray(isThings)
        # print("isThings:",len(isThings))
        if len(isThings)==0:
            return None
        ...
        t_indexes = []
        s_indexes = []
        if len(things_indices):
            num_things = int(0.80 * len(things_indices))
            t_indexes = np.random.choice(things_indices, num_things, replace=False)
        if len(stuff_indices):
            num_stuffs = int(0.40 * len(stuff_indices))
            s_indexes = np.random.choice(stuff_indices, num_stuffs, replace=False)
        if (not things_indices.shape[0]) and (not stuff_indices.shape[0]):
            return None
        
        indxs = np.concatenate([t_indexes, s_indexes]).astype('int')
        # print(indx)
        # print("indxs:", indxs)
        classes = np.array(classes)
        instances.gt_classes = torch.tensor(classes[indxs], dtype=torch.int64)
        all_masks = dataset_dict["padding_mask"].int()
        if len(indxs) == 0 or len(masks)==0:
            ...
            # )
            masks = BitMasks(
                torch.stack([torch.from_numpy(np.ascontiguousarray((masks[i].copy()))).to(dtype =torch.uint8) for i in indxs])
            )
            ...
            if self.random_bg_queries:
                ...
                if pick < 0.20:
                    ...
                    combined_bg_points = torch.max(bg_masks,dim=0).values.to(dtype=torch.uint8)
                    bg_masks = torch.cat((bg_masks,combined_bg_points.unsqueeze(0)))
