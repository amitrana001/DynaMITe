...
class Trainer(DefaultTrainer):
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for i in range(8):
            if i<5:
                if i==0:
                    from mask2former.evaluation.multi_instance_evaluation_per_obj_coordsV0 import evaluate
                elif i==1:
                    from mask2former.evaluation.multi_instance_evaluation_per_obj_coordsV1 import evaluate
                elif i==2:
                    from mask2former.evaluation.multi_instance_evaluation_per_obj_coordsV2 import evaluate
                elif i==3:
                    from mask2former.evaluation.multi_instance_evaluation_per_obj_coordsV3 import evaluate
                elif i==4:
                ...
                from mask2former.evaluation.multi_instance_evaluation_per_obj_coords_rand import evaluate
                for j in range(5):
                    model_name += f"_V{i}_rand{j}"
                    results_i = evaluate(model, data_loader,cfg, dataset_name, sampling_strategy=1)
                    results_i = comm.gather(results_i, dst=0)  # [res1:dict, res2:dict,...]
                    if comm.is_main_process():
                        # sum the values with same keys
                ...
                from mask2former.evaluation.multi_instance_evaluation_per_obj_coords_randv1 import evaluate
                for j in range(5):
                    model_name += f"_V{i}_rand{j}"
                    results_i = evaluate(model, data_loader,cfg, dataset_name, sampling_strategy=1)
                    results_i = comm.gather(results_i, dst=0)  # [res1:dict, res2:dict,...]
                ...
                from mask2former.evaluation.multi_instance_evaluation_per_obj_coords_randv2 import evaluate
                for j in range(5):
                    model_name += f"_V{i}_rand{j}"
                    results_i = evaluate(model, data_loader,cfg, dataset_name, sampling_strategy=1)
                    results_i = comm.gather(results_i, dst=0)  # [res1:dict, res2:dict,...]
...
def default_argument_parser(epilog=None):
    ...
    parser.add_argument("--eval-dataset", type=str, help="perform evaluation on given datset")
